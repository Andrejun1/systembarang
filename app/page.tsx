...
export default function PublicHomePage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "dipinjam" | "kembali">("all");
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(180deg, #020817 0%, #0f1f3d 30%, #0f172a 100%)" }}
    >
      {/* Header */}
      <header
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full overflow-hidden bg-white flex items-center justify-center">
                <img src="/logounimus.png" alt="Logo" className="w-full h-full object-contain" />
              </div>
              <div>
      ...
      <section className="pt-16 pb-10 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-blue-200/60 text-lg max-w-2xl mx-auto mb-10">
            Universitas Muhammadiyah Semarang — Pantau status peminjaman secara real-time
          </p>
      ...
      <section className="px-4 pb-8">
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatsCard label="Total Peminjaman" value={stats.total} icon={Package} color="blue" />
          <StatsCard label="Sedang Dipinjam" value={stats.active} icon={Clock} color="amber" />
          <StatsCard label="Sudah Kembali" value={stats.returned} icon={CheckCircle2} color="emerald" />
          <StatsCard label="Hari Ini" value={stats.today} icon={TrendingUp} color="purple" />
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex gap-2 bg-white/5 p-1.5 rounded-2xl border border-white/10 backdrop-blur-sm">
            ...
            >
              ...
              <span className="px-2 py-0.5 bg-white/20 rounded-full text-xs">{catalogItems.length}</span>
            </button>
            <button
            ...
            >
              ...
              <span className="px-2 py-0.5 bg-white/20 rounded-full text-xs">{loans.length}</span>
            </button>
          </div>
        </div>
      </section>
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                {categories.length > 0 && (
                  ...
                  >
                    ...
                    {categories.map((cat) => (
                      <option key={cat} value={cat as string} className="bg-slate-800">
                        {cat}
                      </option>
                    ))}
                  </select>
              ...
              {catalogLoading ? (
                ...
                <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/10">
                  ...
                  <p className="text-blue-300/70 text-lg font-medium">Tidak ada barang ditemukan</p>
                  <p className="text-blue-400/40 text-sm mt-1">
                    {catalogSearchQuery ? "Coba kata kunci lain" : "Belum ada barang tersedia"}
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {filteredCatalogItems.map((item) => (
                    <div
                      key={item.id}
                      onClick={() =>
                        item.foto_url ? openImageModal(item.foto_url, item.nama_barang) : null
                      }
                    ...
                    >
                      ...
                      <div className="p-4">
                        <h3 className="text-white font-semibold text-sm line-clamp-1">{item.nama_barang}</h3>
                        <p className="text-blue-300/60 text-xs mt-0.5">{item.kode_barang}</p>
                        {item.kategori && (
                        ...
                        <div className="mt-3">
                          <div className="flex justify-between text-[10px] text-blue-300/50 mb-1">
                            ...
                            <span>Sisa: {item.stok_tersedia} dari {item.stok_total}</span>
                          </div>
              ...
              {filteredCatalogItems.length > 0 && (
                <div className="text-center text-blue-300/50 text-sm">
                  Menampilkan {filteredCatalogItems.length} dari {catalogItems.length} barang
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex gap-2">
                  {(["all", "dipinjam", "kembali"] as const).map((s) => (
                    ...
                    >
                      {s === "all" ? "Semua" : s === "dipinjam" ? "Dipinjam" : "Kembali"}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/10">
                  ...
                  <p className="text-blue-300/70 text-lg font-medium">Tidak ada data peminjaman</p>
                  <p className="text-blue-400/40 text-sm mt-1">
                    {loanSearchQuery ? "Coba kata kunci lain" : "Belum ada peminjaman tercatat"}
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {showScanner && (
        <BarcodeScannerModal onResult={handleScanResult} onClose={handleCloseScanner} />
      )}
    </div>
  );
}